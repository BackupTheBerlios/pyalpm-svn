...
cimport alpm
from alpm cimport alpm_strerror, pm_errno
...
class LockError (Exception):
				def __str__ (self):
								return "unable to init transaction: %s" %alpm_strerror(pm_errno)
...
class SetOptError (Exception):
				def __init__ (self, what):
								self.what = what
				def __str__ (self):
								return "cannot set parameter %s: %s" %(self.what, alpm_strerror(pm_errno))
